...
export default function PatientFirstHospitality() {
  ...
  // Patient Profile (Default: Ananya as requested by prompt)
  ...
  const [patientPincode, setPatientPincode] = useState("560076")
  const [locationSource, setLocationSource] = useState<"GPS_AUTO" | "MANUAL_PINCODE" | "POLICY_EXTRACTED">("GPS_AUTO")
  const [detectingLocation, setDetectingLocation] = useState(false)
  const [gpsCoords, setGpsCoords] = useState<{ lat: number; lng: number } | null>({ lat: 12.8943, lng: 77.5989 })
  ...
  // Multi-Facility Selection State (Core Feature)
  ...
  // Default pre-selected: Cardiology + ICU + Cath Lab
  const [selectedFacilities, setSelectedFacilities] = useState<string[]>(["cardiology", "icu", "cath_lab"])
  ...
  // Matched Hospitals Data with distance calculation
  const hospitalList: HospitalOption[] = [
    {
      id: 1,
      name: "Apollo Hospitals",
      city: "Bengaluru",
      pincode: "560076",
      address: "Bannerghatta Road, Bengaluru (PIN 560076)",
      distance_km: 3.2,
      match_score: 94,
      care_fit_score: 94,
      ...
      facilities: [
        ...
        { name: "Cardiac OT", status: "AVAILABLE" }
      ],
      room_compatibility: "Semi-Private within policy limit",
      indicative_cost: 80000,
      reasons: [
        "Cardiology, ICU, and Cath Lab confirmed available",
        "Empanelled on Star Health Cashless Preferred Network",
        "Semi-Private room (₹4,500/day) fully covered under ₹5,000 cap",
        "Proximity: 3.2 km from your location (PIN 560076)"
      ],
      score_breakdown: {
        facility_match: 100,
        network_compatibility: 100,
        room_fit: 100,
        bed_availability: 80,
    ...
    },
    {
      ...
      city: "Bengaluru",
      pincode: "560008",
      address: "Old Airport Road, Bengaluru (PIN 560008)",
      distance_km: 7.8,
      match_score: 86,
      care_fit_score: 86,
      ...
      facilities: [
        { name: "Cardiology", status: "AVAILABLE" },
        { name: "ICU", status: "AVAILABLE" },
        { name: "Cath Lab", status: "VERIFY", note: "Under maintenance check" },
        { name: "Emergency", status: "AVAILABLE" }
      ],
      room_compatibility: "Semi-Private within policy limit",
      indicative_cost: 84000,
      reasons: [
        "Cardiology & ICU confirmed available",
        "Cath Lab availability requires TPA pre-admission verification",
        "Cashless empanelled with Star Health",
        "Proximity: 7.8 km from your location"
      ],
      score_breakdown: {
        facility_match: 85,
        network_compatibility: 100,
    ...
    },
    {
      ...
      city: "Bengaluru",
      pincode: "560069",
      address: "Jayanagar 9th Block, Bengaluru (PIN 560069)",
      distance_km: 4.1,
      match_score: 91,
      care_fit_score: 91,
      match_status: "FULL MATCH",
      ...
      room_compatibility: "General & Semi-Private within limit",
      indicative_cost: 62000,
      reasons: [
        "Apex Government Cardiac Institute",
        "All 3 requested facilities fully operational",
        "Subsidized tariff card minimizes out-of-pocket share",
        "Proximity: 4.1 km from your location"
      ],
      score_breakdown: {
        facility_match: 100,
        network_compatibility: 90,
        room_fit: 100,
        bed_availability: 90,
        cost_compatibility: 95,
        data_confidence: 90
      }
    }
  ]

  // Selected Hospital State (Default: Apollo Hospitals)
  const [selectedHospital, setSelectedHospital] = useState<HospitalOption>(hospitalList[0])
  ...
  // Facility Capability Map Node Click
  ...
  const [chatMessages, setChatMessages] = useState<Array<{ role: string; content: string; citations?: any[] }>>([
    {
      role: "assistant",
      content: `Hello Ananya! I am ASK HOSPITALITY. I am actively tracking your Star Health policy (₹5,00,000 SI), your current location in ${patientCity} (PIN ${patientPincode}), your requested care (${selectedFacilities.map(f => f.toUpperCase()).join(" + ")}), and ${selectedHospital.name}. How can I assist your admission decision today?`
    }
  ])
    ...
    setTimeout(() => {
      ...
      if (q.includes("private room") || q.includes("why is private room not recommended") || q.includes("not recommended")) {
        reply = `**Why Single Private Room is not recommended for Ananya:**\n\n` +
          ...
          `2. **Actual Tariff**: A Single Private Room at ${selectedHospital.name} costs **₹8,000/day** (₹3,000/day over the limit).\n` +
          `3. **IRDAI Proportionate Deduction Penalty**: Choosing this room triggers a proportionate factor of $\\gamma = 5,000 / 8,000 = 0.625$. The insurer will deduct **37.5% penalty (₹16,875)** from doctor, surgeon, and OT charges!\n` +
          `4. **Financial Impact**: Your out-of-pocket responsibility jumps from **₹21,800** in Semi-Private to **₹48,875** in Private Room.\n\n` +
          `**Recommendation**: Stay in the **Semi-Private Room (₹4,500/day)** to enjoy 100% admissible coverage with zero proportionate penalty.`
        citations = [
          { source: "Uploaded Policy Document", clause: "Section 3.1: Room Rent Cap (1% of SI)", page: 7 },
          { source: "Apollo Hospitals Tariff Card", clause: "Single Private Room vs Semi-Private Schedule", page: 1 }
        ]
      } else if (q.includes("recommend") || q.includes("why was this hospital recommended") || q.includes("apollo")) {
        reply = `**Why ${selectedHospital.name} was recommended for Ananya:**\n\n` +
          `• **100% Need Coverage**: Provides all 3 of your selected facilities: **Cardiology**, **ICU**, and **Cath Lab**.\n` +
          `• **Network Empanelment**: Empanelled on Star Health's **Cashless Preferred Network** for instant electronic pre-authorisation.\n` +
          `• **Room Compatibility**: Semi-Private rooms (₹4,500/day) fall safely below your ₹5,000 daily cap.\n` +
          `• **Proximity**: Located ${selectedHospital.distance_km} km from your registered location in ${patientCity} (PIN ${patientPincode}).\n` +
          `• **Bed Availability**: 3 Semi-Private beds and 3 ICU beds currently unoccupied (Simulated Telemetry).`
        citations = [
          { source: "ABDM Health Facility Registry", clause: "Facility Services Registry", page: 1 },
        ...
        reply = `**Actionable Pre-Admission Checklist for Ananya at ${selectedHospital.name}:**\n\n` +
        ...
        reply = `Based on Ananya's Star Health policy (₹5,00,000 SI) at ${selectedHospital.name} (${selectedHospital.distance_km} km from ${patientPincode}), you are currently configured for **${currentRoom.label}** with an indicative out-of-pocket responsibility of **₹${financials.patientOutofPocket.toLocaleString("en-IN")}**. Is there a specific procedure, room overage rule, or pre-auth step you would like to explore?`
      }
              ...
              {analyzingPolicy && (
                <div className="bg-[#0A1226] rounded-2xl p-4 border border-slate-700/60 space-y-2 text-xs">
                  ...
                  <div className={`flex items-center gap-2.5 ${analysisStep >= 3 ? "text-teal-400 font-semibold" : "text-slate-500"}`}>
                    ...
                    <span>3. Identifying Co-payment (10%) & Deductible (₹5,000)</span>
                  </div>
        ...
        <div className="min-h-screen p-6 sm:p-10 max-w-5xl mx-auto space-y-8 animate-fadeIn">
          ...
          <div className="space-y-2">
            ...
            <p className="text-slate-400 text-sm sm:text-base max-w-2xl">
              Select the facilities or services you need. We'll find hospitals near <strong>{patientCity} ({patientPincode})</strong> that can provide them while considering your policy constraints.
            </p>
          </div>
          ...
          <div className="bg-[#0F1B38]/90 border border-slate-700/60 rounded-2xl p-6 flex flex-col sm:flex-row items-center justify-between gap-4 shadow-xl">
            ...
            <button
              onClick={() => setActiveStep("HOSPITAL_OPTIONS")}
              ...
              >
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-700/60 pb-5">
                  <div className="space-y-1">
                    ...
                    <div className="flex items-center gap-3 text-xs text-slate-400 flex-wrap">
                      ...
                      <span className="bg-blue-950/80 text-blue-300 border border-blue-500/30 px-2 py-0.5 rounded font-bold">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 text-xs">
                    ...
                    <div className="space-y-1">
                      {hosp.facilities.map((fac, fi) => (
                        <div key={fi} className="flex items-center justify-between text-slate-200">
                          ...
                          <span className={`text-[10px] font-bold ${fac.status === "AVAILABLE" ? "text-emerald-400" : "text-amber-400"}`}>
                            {fac.status === "AVAILABLE" ? "✓ Available" : "⚠ Verify"}
                          </span>
                        </div>
                      ))}
                    </div>
                  ...
                  <div className="bg-[#14234B]/60 border border-slate-700/60 rounded-xl p-3.5 space-y-1.5">
                    ...
                    <p className="text-[11px] text-slate-400">Semi-Private rate ₹4,500 fits ₹5,000 cap</p>
                  </div>
        ...
        <div className="min-h-screen flex">
          ...
          <main className="flex-1 ml-72 p-8 max-w-6xl mx-auto space-y-16">
                  ...
                  <div className="space-y-2 flex-1">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 text-xs text-slate-200 pt-1">
                      <div className="flex items-center gap-1.5 text-emerald-300 font-semibold">
                        <Check className="w-4 h-4 text-emerald-400" /> Cardiology Supported
                      </div>
                      <div className="flex items-center gap-1.5 text-emerald-300 font-semibold">
                        <Check className="w-4 h-4 text-emerald-400" /> ICU Supported
                      </div>
                      <div className="flex items-center gap-1.5 text-emerald-300 font-semibold">
                        <Check className="w-4 h-4 text-emerald-400" /> Cath Lab Supported
                      </div>
                      <div className="flex items-center gap-1.5 text-emerald-300 font-semibold">
                ...
                <div className="flex flex-col items-center space-y-6 max-w-2xl mx-auto">
                  ...
                  <button
                    onClick={() => setSelectedCapabilityNode({
                      ...
                      details: "Comprehensive adult & pediatric cardiology OPD, Echo & non-invasive testing."
                    })}
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 w-full">
                    <button
                      onClick={() => setSelectedCapabilityNode({
                        ...
                        details: "12-bed cardiac CCU with ventilator & hemodynamic monitoring."
                      })}
                    ...
                    >
                      ...
                      <div className="text-[11px] text-slate-400">3 Beds Unoccupied</div>
                    </button>
                    ...
                    <button
                      onClick={() => setSelectedCapabilityNode({
                        ...
                        details: "Flat-panel digital angiography suite for Angioplasty (PTCA) & Stenting."
                      })}
                      className="p-4 bg-emerald-950/70 hover:bg-emerald-900/80 border border-emerald-500/50 rounded-2xl text-center space-y-1 transition-all"
                    >
                      <div className="text-xs font-extrabold text-emerald-300 flex items-center justify-center gap-1.5">
                    ...
                    <button
                      onClick={() => setSelectedCapabilityNode({
                        ...
                        freshness: "Verified 2026",
                        details: "Cardiac emergency resuscitation unit with triage."
                      })}
                      className="p-4 bg-emerald-950/70 hover:bg-emerald-900/80 border border-emerald-500/50 rounded-2xl text-center space-y-1 transition-all"
                    >
                      <div className="text-xs font-extrabold text-emerald-300 flex items-center justify-center gap-1.5">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 w-full max-w-lg">
                    <button
                      onClick={() => setSelectedCapabilityNode({
                        ...
                        freshness: "Verified 2026",
                        details: "Laminar airflow modular surgical suites for open & minimally invasive bypass."
                      })}
                    ...
                    <button
                      onClick={() => setSelectedCapabilityNode({
                        name: "Advanced Cardiac Diagnostics",
                        status: "AVAILABLE",
                        source: "Diagnostic Network Master",
                        freshness: "Verified 2026",
                        details: "Cardiac Troponin I, NT-proBNP, CT Angio 64-slice."
                      })}
                      className="p-3.5 bg-emerald-950/70 hover:bg-emerald-900/80 border border-emerald-500/50 rounded-xl text-center text-xs font-bold text-emerald-300"
                    >
                ...
                <div className="bg-[#0F1B38]/90 border border-emerald-500/30 rounded-3xl p-6 shadow-xl space-y-3">
                  ...
                  <div className="space-y-2 text-slate-200">
                    <div className="p-2.5 bg-emerald-950/40 rounded-xl border border-emerald-500/20">
                      ✓ <strong>Required Facilities:</strong> Cardiology, ICU & Cath Lab operational
                    </div>
                    <div className="p-2.5 bg-emerald-950/40 rounded-xl border border-emerald-500/20">
                ...
                <div className="bg-gradient-to-br from-teal-950 via-[#0F1B38] to-blue-950 border border-teal-500/50 rounded-3xl p-6 shadow-xl flex flex-col justify-between space-y-4">
                  <div>
                    ...
                    <p className="text-slate-300 mt-2 leading-relaxed">
                      {financials.isRoomCapped
                        ? "Switch back to Semi-Private Room to avoid ₹16,875 in proportionate deductions, then initiate cashless pre-authorization."
                        : "Your room, distance (3.2 km), and policy are optimal! Proceed to verify cashless pre-authorization at Apollo Hospitals."}
                    </p>
                  </div>
            ...
            <section id="sources" className="scroll-mt-8 space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-4 gap-3 text-xs">
                {[
                  { name: "data.gov.in", org: "MoHFW", status: "Active (8 Records)" },
          ...
          <div className="bg-[#0F1B38] border border-teal-500/40 rounded-3xl max-w-lg w-full p-6 shadow-2xl space-y-6">
            ...
            <div className="space-y-3 text-xs">
              <div className="flex justify-between items-center p-2.5 bg-[#0A1226] rounded-xl">
                <span className="text-slate-300">Facility Match (Cardiology, ICU, Cath Lab)</span>
              ...
              <div className="flex justify-between items-center p-2.5 bg-[#0A1226] rounded-xl">
                <span className="text-slate-300">Room Rent Limit Fit (₹4,500 vs ₹5,000 cap)</span>
...
with open(target_path, "w", encoding="utf-8") as f:
    f.write(code)
print(f"Successfully generated Live Location-Aware UX to {target_path}")
