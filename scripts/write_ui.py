    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 flex">
      ...
      <aside className="w-72 bg-slate-900 text-slate-100 flex flex-col fixed inset-y-0 left-0 z-50 shadow-xl">
        ...
        <div className="p-4 border-t border-slate-800 bg-slate-950/60 text-xs space-y-2">
          ...
          <div className="text-[11px] text-slate-400 leading-tight">
            Decision-support only. Non-binding indicative estimates.
          </div>
        </div>
      </aside>
      ...
      <main className="flex-1 ml-72 p-8 max-w-6xl mx-auto space-y-16">
        ...
        <section id="start-policy" className="scroll-mt-8 space-y-6">
          <div className="border-b border-slate-200 pb-4">
            ...
            <p className="text-slate-600 text-sm mt-1">
              Upload your health insurance policy certificate or schedule (PDF or JSON). HOSPITALITY extracts your constraints without manual data entry.
            </p>
          </div>
              ...
              {uploadedFile ? (
                ...
                <div className="flex gap-3">
                  ...
                  <button 
                    ...
                    className="bg-blue-50 hover:bg-blue-100 text-blue-700 border border-blue-200 text-xs font-semibold px-4 py-2.5 rounded-lg transition-all"
                  >
        ...
        <section id="dashboard" className="scroll-mt-8 space-y-6">
          <div className="border-b border-slate-200 pb-4">
            <span className="text-xs font-bold uppercase tracking-wider text-blue-600 bg-blue-50 px-2.5 py-1 rounded-full border border-blue-200">
              Stage 2 — Dynamic Overview
            </span>
            <h2 className="text-3xl font-extrabold text-slate-900 mt-2">Patient Decision Dashboard</h2>
            <p className="text-slate-600 text-sm mt-1">
              Dynamic summary populated directly from your extracted policy constraints and matched healthcare network.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
            <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm flex flex-col justify-between">
              ...
              <div>
                <div className="text-xl font-black text-slate-900">
                  {policy ? policy.policy_number : "No policy uploaded"}
                </div>
                <p className="text-xs text-slate-500 mt-1 font-medium">
                  {policy ? `${policy.insurer}` : "Upload policy above to activate"}
                </p>
              </div>
            ...
            <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm flex flex-col justify-between">
              ...
              <div>
                <div className="text-xl font-black text-slate-900">
                  {hospitals.length > 0 ? `${hospitals.length} Facilities` : "—"}
                </div>
                <p className="text-xs text-slate-500 mt-1 font-medium">
                  {hospitals.length > 0 ? "Cashless empanelled in Bengaluru" : "Run analysis to rank"}
                </p>
              </div>
            ...
            <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm flex flex-col justify-between">
              ...
              <div>
                <div className="text-xl font-black text-slate-900">
                  {hospitals.length > 0 ? `${hospitals[0].available_beds} Available` : "—"}
                </div>
                <p className="text-xs text-slate-500 mt-1 font-medium">
                  {hospitals.length > 0 ? `${hospitals[0].available_icu_beds} ICU beds unoccupied` : "Live bed feed standby"}
                </p>
              </div>
            ...
            <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm flex flex-col justify-between">
              ...
              <div>
                <div className="text-xl font-black text-slate-900">
                  {deductionResult ? `₹${deductionResult.indicative_patient_out_of_pocket.toLocaleString("en-IN")}` : "—"}
                </div>
                <p className="text-xs text-slate-500 mt-1 font-medium">
                  {deductionResult ? `${deductionResult.room_category.replace("_", " ")} Room` : "Based on selected room"}
                </p>
              </div>
        ...
        <section id="policy-analysis" className="scroll-mt-8 space-y-6">
          <div className="border-b border-slate-200 pb-4">
            <span className="text-xs font-bold uppercase tracking-wider text-blue-600 bg-blue-50 px-2.5 py-1 rounded-full border border-blue-200">
              Stage 3 — Policy Understanding
            </span>
            <h2 className="text-3xl font-extrabold text-slate-900 mt-2">Policy Constraints & Clause Citations</h2>
            <p className="text-slate-600 text-sm mt-1">
              Deterministic parameters parsed from the insurance schedule with exact source page citations.
            </p>
          </div>
              ...
              <div className="lg:col-span-2 bg-white rounded-2xl border border-slate-200 p-6 shadow-sm space-y-5">
                <h3 className="font-bold text-slate-900 text-base flex items-center justify-between">
                  ...
                  <span className="text-xs bg-emerald-100 text-emerald-800 font-bold px-2.5 py-0.5 rounded-full">
                    {policy.extraction_confidence * 100}% Confidence Score
                  </span>
                </h3>
                  ...
                  {policy.clauses.map((c, i) => (
                    <div key={i} className="p-3 bg-slate-50 rounded-xl border border-slate-200 text-xs flex items-start justify-between gap-3">
                      <div className="space-y-1">
                        <span className="font-bold text-blue-700 bg-blue-100/70 px-1.5 py-0.5 rounded text-[10px] mr-2">
                          PAGE {c.page}
                        </span>
        ...
        <section id="hospitals" className="scroll-mt-8 space-y-6">
          <div className="border-b border-slate-200 pb-4">
            <span className="text-xs font-bold uppercase tracking-wider text-blue-600 bg-blue-50 px-2.5 py-1 rounded-full border border-blue-200">
              Stage 4 — Hospital Discovery
            </span>
            <h2 className="text-3xl font-extrabold text-slate-900 mt-2">Compatible Empanelled Hospitals</h2>
            <p className="text-slate-600 text-sm mt-1">
              Multi-criteria matching scores based on Cashless Network status, Room Rent fit, Distance, and Live Bed Availability.
            </p>
          </div>
            ...
            <div className="space-y-4">
              {hospitals.map((h, idx) => (
                <div 
                  ...
                  className={`p-6 rounded-2xl border transition-all cursor-pointer bg-white shadow-sm flex flex-col md:flex-row items-start md:items-center justify-between gap-6 ${
                    selectedHospitalId === h.id ? "border-blue-600 ring-2 ring-blue-600/20 shadow-md" : "border-slate-200 hover:border-slate-300"
                  }`}
                >
                  <div className="space-y-2 flex-1">
                    <div className="flex items-center gap-3 flex-wrap">
                      ...
                      <span className={`text-xs font-bold px-2.5 py-0.5 rounded-full ${
                        h.network_status === "CASHLESS_NETWORK" ? "bg-emerald-100 text-emerald-800" : "bg-amber-100 text-amber-800"
                      }`}>
                    ...
                    <div className="flex flex-wrap gap-2 pt-1">
                      {h.reasons.map((r, ri) => (
                        <span key={ri} className="text-[11px] bg-slate-100 text-slate-700 font-medium px-2.5 py-0.5 rounded-md">
                          • {r}
                        </span>
                      ))}
                    </div>
                  ...
                  <div className="flex md:flex-col items-center md:items-end justify-between w-full md:w-auto gap-4 border-t md:border-t-0 pt-3 md:pt-0 border-slate-100 shrink-0">
                    ...
                    <div className="text-right">
                      <span className="text-[11px] text-purple-800 font-bold bg-purple-100 px-2 py-0.5 rounded">
                        {h.available_beds} Beds Free ({h.available_icu_beds} ICU)
                      </span>
                    </div>
                  </div>
                </div>
        ...
        <section id="coverage" className="scroll-mt-8 space-y-6">
          <div className="border-b border-slate-200 pb-4">
            <span className="text-xs font-bold uppercase tracking-wider text-blue-600 bg-blue-50 px-2.5 py-1 rounded-full border border-blue-200">
              Stage 5 — Cost & Coverage Intelligence
            </span>
            <h2 className="text-3xl font-extrabold text-slate-900 mt-2">Proportionate Deduction Simulator</h2>
            <p className="text-slate-600 text-sm mt-1">
              Interactive mathematical simulator demonstrating how room category selection triggers or prevents IRDAI proportionate deduction penalties.
            </p>
          </div>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm space-y-6">
            ...
            {deductionResult ? (
                ...
                <div className={`p-4 rounded-xl border flex items-start gap-3 text-sm ${
                  ...
                  <div>
                    <h4 className="font-bold">
                      {deductionResult.is_room_capped 
                        ? "Proportionate Deduction Penalty Triggered!" 
                        : "Zero Room Rent Penalty — Optimal Selection!"}
                    </h4>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-xs">
                  <div className="space-y-3 bg-slate-50 p-4 rounded-xl border border-slate-200">
                    ...
                    <div className="flex justify-between py-1 border-b border-slate-200">
                      <span className="text-slate-600">Room Rent ({deductionResult.days_of_stay} days @ ₹{deductionResult.actual_room_tariff_per_day:,.0f})</span>
              ...
              <div className="text-center py-6 text-slate-400 text-xs">
                Run policy analysis in Stage 1 to activate calculation proof.
              </div>
            )}
          </div>
        </section>
        ...
        <section id="journey" className="scroll-mt-8 space-y-6">
          <div className="border-b border-slate-200 pb-4">
            <span className="text-xs font-bold uppercase tracking-wider text-blue-600 bg-blue-50 px-2.5 py-1 rounded-full border border-blue-200">
              Stage 6 — Care Journey Tracking
            </span>
            <h2 className="text-3xl font-extrabold text-slate-900 mt-2">Patient Journey State Machine</h2>
            <p className="text-slate-600 text-sm mt-1">
              Reactive lifecycle guidance advancing through admission milestones with pre-auth checklists.
            </p>
          </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-6 gap-2 text-center text-xs">
              {[
                ...
                <button
                  ...
                  className={`p-3 rounded-xl border font-bold transition-all text-left sm:text-center ${
                    currentJourneyStage === st.id
        ...
        <section id="chat" className="scroll-mt-8 space-y-6">
          <div className="border-b border-slate-200 pb-4">
            <span className="text-xs font-bold uppercase tracking-wider text-blue-600 bg-blue-50 px-2.5 py-1 rounded-full border border-blue-200">
              Stage 7 — Patient Conversational AI
            </span>
            <h2 className="text-3xl font-extrabold text-slate-900 mt-2">Tool-Augmented Healthcare Assistant</h2>
            <p className="text-slate-600 text-sm mt-1">
              Ask natural-language questions about your policy coverage, room limits, and hospital billing.
            </p>
          </div>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm flex flex-col h-[450px]">
                  ...
                  {m.citations && m.citations.length > 0 && (
                    <div className="mt-1 flex gap-2">
                      {m.citations.map((c, ci) => (
                        <span key={ci} className="text-[10px] bg-slate-200 text-slate-700 px-2 py-0.5 rounded font-bold">
                          📖 {c.source} ({c.clause})
                        </span>
                      ))}
                    </div>
            ...
            <div className="px-6 py-2 bg-slate-50 border-t border-slate-100 flex gap-2 overflow-x-auto text-[11px]">
              <button 
                onClick={() => setChatInput("Is a private room covered at Apollo?")}
                className="bg-white hover:bg-slate-100 border border-slate-200 px-2.5 py-1 rounded-full text-slate-700 whitespace-nowrap"
              >
                "Is private room covered at Apollo?"
              </button>
              <button 
                onClick={() => setChatInput("Why does Deluxe room cost more out of pocket?")}
                className="bg-white hover:bg-slate-100 border border-slate-200 px-2.5 py-1 rounded-full text-slate-700 whitespace-nowrap"
              >
                "Why does Deluxe room cost more?"
              </button>
              <button 
                onClick={() => setChatInput("Which hospitals in Bangalore have cashless for Star Health?")}
                className="bg-white hover:bg-slate-100 border border-slate-200 px-2.5 py-1 rounded-full text-slate-700 whitespace-nowrap"
              >
            ...
            <form onSubmit={handleSendMessage} className="p-4 border-t border-slate-200 flex gap-3">
              ...
              <button
                ...
                className="bg-blue-600 hover:bg-blue-700 text-white px-5 py-2.5 rounded-xl font-bold text-xs shadow-md shadow-blue-600/30 transition-all flex items-center gap-1.5"
              >
        ...
        <section id="audit" className="scroll-mt-8 space-y-6">
          <div className="border-b border-slate-200 pb-4">
            ...
            <p className="text-slate-600 text-sm mt-1">
              Transparent tagging distinguishing authoritative government registries from algorithmic simulations.
            </p>
          </div>
        ...
        <section id="sources" className="scroll-mt-8 space-y-6">
          <div className="border-b border-slate-200 pb-4">
            <span className="text-xs font-bold uppercase tracking-wider text-blue-600 bg-blue-50 px-2.5 py-1 rounded-full border border-blue-200">
              Stage 9 — Ecosystem Catalog
            </span>
            <h2 className="text-3xl font-extrabold text-slate-900 mt-2">Integrated Healthcare Data Sources</h2>
            <p className="text-slate-600 text-sm mt-1">
              Federated Indian digital health registries and standardized master datasets.
            </p>
          </div>
        ...
        <section id="fhir" className="scroll-mt-8 space-y-6 pb-16">
          <div className="border-b border-slate-200 pb-4">
            <span className="text-xs font-bold uppercase tracking-wider text-blue-600 bg-blue-50 px-2.5 py-1 rounded-full border border-blue-200">
              Stage 10 — Interoperability Standards
            </span>
            <h2 className="text-3xl font-extrabold text-slate-900 mt-2">HL7 FHIR R4 & NHCX Interoperability Viewer</h2>
            <p className="text-slate-600 text-sm mt-1">
              Standardized electronic claim and eligibility payloads compliant with NRCeS ABDM standards.
            </p>
          </div>
...
with open(target_path, "w", encoding="utf-8") as f:
    f.write(ui_code)
print("File written cleanly to:", target_path)
